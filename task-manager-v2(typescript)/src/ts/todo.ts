document.addEventListener("DOMContentLoaded", () => {
  const task_list: [string, boolean][] = [];
  const cover = document.getElementById("cover") as HTMLInputElement;
  const no_image = document.getElementById("no_image") as HTMLInputElement;
  const add_task = document.getElementById("add_task") as HTMLInputElement;
  const add_task_note = document.getElementById(
    "add_task_note"
  ) as HTMLInputElement;
  const tasks = document.getElementById("tasks") as HTMLInputElement;

  const taskAvailbilty = (): void => {
  ...
  const completeMarker = (): void => {
    document.querySelectorAll(".container").forEach((item) => {
      const marker = item.getAttribute("data-completed");
      const text = item.querySelector(".left .task") as HTMLElement;
      const checkbox = item.querySelector(".left .check") as HTMLInputElement;
      if (marker == "true") {
  ...
  const taskAdder = (): void => {
    ...
    document.querySelectorAll(".check").forEach((checkbox) => {
      checkbox.addEventListener("click", () => {
        ...
        const leftcontainer = checkbox.closest(".left") as HTMLElement;
        const taskNote = leftcontainer.querySelector(
          ".task"
        ) as HTMLInputElement;
  ...
  const addTask = (): void => {
    ...
    apply.addEventListener("click", () => {
      const adding = document.getElementById("adding") as HTMLInputElement;
  ...
  taskAvailbilty();
  const editInput = (index: number): void => {
    cover.classList.remove("disabled");
    cover.classList.add("cover");
    add_task_note.innerHTML = `
        <div id = "popup">
  ...
  const deleteTask = (index: number): void => {
    ...
    const cancel = document.getElementById("cancel") as HTMLInputElement;
