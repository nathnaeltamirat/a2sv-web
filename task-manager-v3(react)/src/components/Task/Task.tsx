...
const Task = () => {
  ...
  const taskAdder = (task: string) => {
    setTaskList([...taskList, [task, false]]);
  };
const toggler = (toogle:boolean)=>{
    setEdit(toogle)
  }
  const editTask = (inputValue:string,index:number)=>{
        setTaskList((prev) =>
      prev.map((task, i) => {
        return index == i ? [inputValue, task[1]] : task;
      })
    );
  ...
  const toggleComplete = (index: number) =>
    setTaskList((prev) =>
      prev.map((task, i) => {
        return index == i ? [task[0], !task[1]] : task;
      })
    );
...
setEdittingIndex(index)
  ...
  const Tasks = taskList.map((value, index) => {
    return (
      <>
        {console.log(value)}
        <div className="container">
          <div className="left">
            <input
              ...
              type="checkbox"
            />
            <p className={value[1] ? "task completed" : "task "}> {value[0]}</p>
          </div>
          <div className="right">
            <svg
              data-index={index}
        ...
        <hr />
        {index == edittingIndex && <Edit toggler ={edit} pos={index} onToggler={toggler} inputValue = {value[0]} onEditTask = {editTask} />}
          </>
    );
  });
  return (
    <>
      {taskList.length > 0 ? (
        ...
        <img src="no-image.png" className="no-image" />
      )}
      <Adder onAddTask={taskAdder} />
