...
export const data = async (): Promise<Opportunity[]>=> {
  ...
  const bookmarks = await fetch('https://akil-backend.onrender.com/bookmarks',{
    headers:{
  ...
  const json_bookmrks = await bookmarks.json();
  const saving = ()=>{
      const eventIds = json_bookmrks.data.map((item:any)=>item.eventID)
